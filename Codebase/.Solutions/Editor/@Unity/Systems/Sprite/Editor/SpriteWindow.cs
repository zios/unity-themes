...
namespace Zios.Unity.Editor.SpriteManager{
	...
	public class SpriteAssets{
		...
		public SpriteAssets(){
			this.grid = File.GetAsset<Texture2D>("TransparencyGray.png");
			this.gridBlue = File.GetAsset<Texture2D>("TransparencyBlue.png");
			this.gridSilver = File.GetAsset<Texture2D>("TransparencySilver.png");
			this.UI = File.GetAsset<GUISkin>("Sprite.guiskin");
			this.spriteMesh = File.GetAsset<Mesh>("Sprite.fbx");
	...
	public class SpriteWindow : EditorWindow{
		...
		public void DelayRender(){
			if(this.disabled || Proxy.IsPlaying()){return;}
		...
		public void ResetSystem(){
			Log.Show("-------------------------------");
			Log.Show("[SpriteWindow] Resetting system");
			File.Refresh();
			...
			if(SpriteManager.sequences.Count > 0){
				...
				string lastSprite = PlayerPref.Get<string>("SpriteWindow-Selected");
		...
		public void CheckHotkeys(){
			...
			if((control||alt||shift) && mouseMove){
				if(this.controlPosition == Vector2.zero){
					ProxyEditor.RegisterUndo(Selection.transforms,"Brush Operation");
			...
			foreach(Transform active in selected){
				...
				if(CheckKey(KeyCode.Z)){
					ProxyEditor.RegisterUndo(Selection.transforms,"Zero Position");
		...
		public void FixScene(bool allowForce = false){
			Log.Show("[SpriteWindow] Fixing Scene = " + (allowForce?"Full":"Repair"));
			...
			Log.Show("[SpriteWindow] " + repairList.Count + " Sprites found. "  + sprites.Count + " Materials found.");
			...
			if(current != null && isSprite){
				if(current.GetComponent<Renderer>() != null){
					if(current.GetComponent<Renderer>().sharedMaterial == null){
						...
						string sourcePath = File.GetAssetPath(sprite.parent.xml);
						sourcePath = this.FindAssetFolder(sourcePath,sprite.name,"Materials");
						string materialPath = sourcePath+fullName+".mat";
						current.GetComponent<Renderer>().sharedMaterial = File.GetAsset<Material>(materialPath);
						if(current.GetComponent<Renderer>().sharedMaterial == null){
							Log.Error("[SpriteWindow] Material assignment could not be fixed -- " + name);
						}
						else{
							Log.Show("[SpriteWindow] Fixed material assignment -- " + name);
						}
					}
				...
				Log.Error("[SpriteWindow] Object was destroyed while attempting to repair -- " + name);
			}
		}
		//===========================
		...
		public void LoadPalettes(){
			foreach(var data in SpriteManager.spriteSheets){
				...
				FileData paletteImage = File.Find(paletteName);
		...
		public void SeparateAtlas(SpriteSheet sheet){
			/*string path = File.Find(sheet.imagePath).GetFolderPath() + sheet.imagePath.Replace("Atlas","Lookup");
		...
		public void CreateMaterial(Sprite sprite,bool forceOverwrite=false,bool forceUpdate=false){
			...
			foreach(string name in names){
				...
				if(!File.Exists(materialPath) || forceOverwrite || forceUpdate){
					if(forceOverwrite){
						ProxyEditor.DeleteAsset(materialPath);
					}
					this.ProgressReport(name+".mat");
					bool existed = true;
					Material spriteMaterial = File.GetAsset<Material>(materialPath);
					if(spriteMaterial == null){
						Log.Show("[SpriteWindow] Creating Material -- " + name);
						existed = false;
						spriteMaterial = new Material(spriteShader);
					}
					if(this.ApplyMaterial(spriteMaterial,sprite)){
						Log.Show("[SpriteWindow] Updating Material -- " + name);
						//spriteMaterial.shader = spriteShader;
						if(!existed){ProxyEditor.CreateAsset(spriteMaterial,materialPath);}
						else{
					...
					Log.Show("[SpriteWindow] Loading Material -- " + name);
					this.spriteMaterials[name] = File.GetAsset<Material>(materialPath);
					if(this.spriteMaterials[name] == null){
						Log.Warning("[SpriteWindow] Material could not be loaded -- " + name);
						this.spriteMaterials.Remove(name);
					}
				}
				if(this.spriteMaterials.ContainsKey(name) && this.spriteMaterials[name].name != name){
					Log.Show("[SpriteWindow] Fixing Material Name -- " + name);
		...
		public void CreateMesh(Sprite sprite,bool forceOverwrite=false,bool forceUpdate=false){
			...
			foreach(string name in names){
				...
				if(!File.Exists(meshPath) || forceOverwrite || forceUpdate){
					if(forceOverwrite){
						ProxyEditor.DeleteAsset(meshPath);
					}
					Log.Show("[SpriteWindow] Creating Mesh -- " + name);
					this.ProgressReport(name+".asset");
					bool existed = true;
					Mesh spriteMesh = File.GetAsset<Mesh>(meshPath);
					if(spriteMesh == null){
						existed = false;
					...
					Log.Show(sprite.current.bounds.ToString());
					Log.Show(sprite.current.fullBounds.ToString());
					...
					if(!existed){ProxyEditor.CreateAsset(spriteMesh,meshPath);}
					this.spriteMeshes[name] = spriteMesh;
					ProxyEditor.RefreshAssets();
				}
				else{
					Log.Show("[SpriteWindow] Loading Mesh -- " + name);
					this.spriteMeshes[name] = File.GetAsset<Mesh>(meshPath);
				}
			}
		}
		public void CreatePrefab(Sprite sprite,bool forceOverwrite=false,bool forceUpdate=false){
			...
			foreach(string name in names){
				...
				if(!File.Exists(prefabPath) || forceOverwrite || forceUpdate){
					if(forceOverwrite){
						ProxyEditor.DeleteAsset(prefabPath);
					}
					bool adjustScale = this.autoScale;
					GameObject spritePrefab;
					GameObject scalePrefab;
					UnityObject prefab = File.GetAsset<GameObject>(prefabPath);
					if(prefab == null){
						...
						if(!this.spriteMaterials.ContainsKey(name)){
							Log.Error("[SpriteWindow] Material for prefab does not exist -- " + name);
							return;
						}
						if(this.spriteMaterials[name] == null){
							this.spriteMaterials.Remove(name);
							Log.Error("[SpriteWindow] Material key for prefab was corrupt.  Please try again -- " + name);
							return;
						}
						if(sprite.animated){
						...
						Log.Show("[SpriteWindow] Updating Prefab -- " + name);
					}
					if(adjustScale){
				...
				Log.Show("[SpriteWindow] Loading Prefab -- " + name);
				string path = prefabPath;
				this.spritePrefabs[name] = File.GetAsset<GameObject>(path);
				if(this.spritePrefabs[name] == null){
					...
					if(deleted){
						...
						Log.Error("[SpriteWindow] Failed loading prefab -- " + path);
					}
				}
			}
		}
		...
		public void ResetBrush(){
			Log.Show("[SpriteWindow] Resetting Brush");
		...
		public GameObject ApplySprite(GameObject target = null,Sprite sprite = null,bool fixPath = true,bool allowGeneration = true){
			...
			if(this.assetsReady){
				...
				Log.Show("[SpriteWindow] Applying Sprite -- " + sprite.fullName + activeName);
			}
			...
			if((mismatchedAtlas || noBrushMaterial)){
				Log.Show("[SpriteWindow] Mismatch = " + mismatchedAtlas + " | No Brush Material = " + noBrushMaterial);
			...
			if(!this.spriteMaterials.ContainsKey(spriteName) || !this.spritePrefabs.ContainsKey(spriteName)){
				...
				Log.Warning("[SpriteWindow] " + type + " keys do not exist -- " + spriteName);
				return target;
			}
			foreach(GameObject current in objects){
				if(current != null){
					...
					if(spriteMaterial == null || spritePrefab == null){
						...
						Log.Error("[SpriteWindow] Problem finding/creating " + type + " -- " + spriteName);
						return target;
					}
					if(this.brush == current){
						...
						Log.Warning("[SpriteWindow] Cannot apply atlas selection.  Object is not a sprite nor does it have an atlas-capable shader -- " + current.name);
					}
				}
			}
		...
		public bool ApplyMaterial(Material material,Sprite sprite){
			...
			if(changed && material.shader != this.assets.shaderEditor){
				Log.Show("[SpriteWindow] Material properties changed -- " + material.name);
			}
			return changed;
		}
		...
		public void DrawScene(SceneView view){
			...
			if(this.placementMode && brushUsable && !Proxy.IsRepainting() && Event.current.type != EventType.Layout){
				bool alt = Event.current.alt;
				bool control = Event.current.control;
				bool shift = Event.current.shift;
				bool mouseMove = Event.current.type == EventType.MouseMove;
		...
		public void SetupBakes(bool bake){
			...
			foreach(GameObject current in objects){
				...
				if(bake && filter.sharedMesh == this.assets.spriteMesh && current.GetComponent<Renderer>() != null){
					...
					FileData materialFile = File.Find(filter.sharedMesh.name+".mat");
		...
		public void BakeSprite(GameObject current,bool bake){
			...
			if(bake){
				...
				FileData meshFile = File.Find(renderer.sharedMaterial.name+".asset");
				if(meshFile != null){
					...
					FileData materialFile = File.Find(atlasName);
					if(materialFile == null){
						string atlasPath = File.GetAssetPath(sprite.parent.xml);
						atlasPath = atlasPath.GetDirectory() + "/" + atlasName;
						ProxyEditor.CreateAsset(atlasMaterial,atlasPath);
						ProxyEditor.RefreshAssets();
						atlasMaterial = ProxyEditor.LoadMainAsset(atlasPath).As<Material>();
					}
			...
			if(Proxy.IsPlaying()){
				...
				foreach(var sheetData in new Dictionary<string,SpriteSheet>(SpriteManager.spriteSheets)){
					...
					if(GUI.Button(area,"",GUIStyle.none)){
						...
						EditorPref.Set<bool>(sheetData.Key + "Hide",!iscollapsed);
					}
					currentY += 33;
