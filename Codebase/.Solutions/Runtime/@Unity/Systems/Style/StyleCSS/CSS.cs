...
using System.Text;
using UnityEngine;
namespace Zios.Unity.Style.CSS{
	...
	public static class StyleCSS{
		...
		public static void SaveCSS(string path,GUISkin skin){
			...
			foreach(var item in skin.GetVariables<GUIStyle>(null,Reflection.publicFlags)){
				var defaultStyle = emptySkin.GetVariable<GUIStyle>(item.Key);
				output.Append(StyleCSS.SaveCSS(item.Value,defaultStyle));
			}
			foreach(var style in skin.customStyles){
				if(style.IsNull()){continue;}
				output.Append(StyleCSS.SaveCSS(style,null,false));
			}
			if(output.Length > 0){
		...
		public static string SaveCSS(GUIStyle style,GUIStyle empty=null,bool asTag=true){
			...
			var contents = StyleCSS.contents;
			...
			foreach(var item in style.GetVariables<GUIStyleState>(null,Reflection.publicFlags)){
		...
		public static GUISkin LoadCSS(string name,string contents){
			...
			var stateName = "";
			//var textures = File.GetAssets<Texture2D>().ToDictionary(x=>x.name,x=>x);
			//var fonts = File.GetAssets<Texture2D>().ToDictionary(x=>x.name,x=>x);
			var textures = File.GetNamedAssets<Texture2D>();
			var fonts = File.GetNamedAssets<Font>();
