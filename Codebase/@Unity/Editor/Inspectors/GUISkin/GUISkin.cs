...
using UnityEditor;
using UnityEngine;
namespace Zios.Unity.Editor.Inspectors.GUISkin{
	using Zios.Events;
	...
	public partial class GUISkinEditor : Editor{
		...
		public override void OnInspectorGUI(){
			if(Event.current.type.MatchesAny("mouseMove")){return;}
			EditorUI.Reset();
			...
			foreach(var term in this.inputTerms){
				...
				if(attribute.IsEnum<GUIStyleField>()){
					...
					if(lookup == 2){
						...
						var image = value == "UnityWhite" ? Texture2D.whiteTexture : File.GetAsset<Texture2D>(value+".*",false);
					...
					if(lookup == 7){
						...
						var font = File.GetAsset<Font>(value+".ttf",false) ?? File.GetAsset<Font>(value+".otf",false);
		...
		public void DrawStyles(string key,GUIStyle[] styles,bool editable=true){
			...
			for(int index=0;index<styles.Length;++index){
				...
				if(this.CheckBounds(style)){
					...
					if(this.rebuild && Proxy.IsRepainting()){
		...
		public void ProcessMenu(){
			if(Proxy.IsRepainting() && !GUISkinEditor.focus.IsNull() && !GUISkinEditor.action.ContainsAny("copy","paste")){
	...
	public partial class GUISkinEditor{
		...
		public void ResetVisible(){
			...
			this.scroll = ProxyEditor.GetInspectorScroll(this);
		...
		public void CheckChanges(){
			if(this.changes){
				ProxyEditor.SetAssetDirty(this.target.As<GUISkin>());
				foreach(var fragment in fragments){
					ProxyEditor.SetAssetDirty(fragment);
				}
			...
			if(this.optimize){
				...
				if(!this.rebuild){
					if(Proxy.IsRepainting()){
						if(GUI.changed || this.scroll.y != ProxyEditor.GetInspectorScroll(this).y){
							this.ResetVisible();
						}
					}
				}
