Shader "Zios/Utility/Draw Vertexes"{
	Properties{
		[Header(Vertex)]
		...
		displayShadeColor("Color Shade",Color) = (0,0.0235,0.2509,1)
		...
		Pass{
			...
			void AddQuad(inout TriangleStream<fragmentInput> stream,vertexInput input){
				...
				float4 origin = vertex.position;
					...
					float3 xSize = displaySize + 0.009f;
			...
			fixed4 pixelPass(fragmentInput input) : SV_Target{
				if(distance(input.color,float4(1,1,1,1)) > 0){
					...
					displayShadeColor = input.color * 0.3f;
					displayShadeColor.a = input.color.a;
				}
				float centerDistance = distance(input.uv,float2(0.5,0.5));
				fixed shadeBias = 1/displayRoundness * 0.75f;
				fixed4 activeColor = lerp(displayColor,displayShadeColor,centerDistance*shadeBias);
