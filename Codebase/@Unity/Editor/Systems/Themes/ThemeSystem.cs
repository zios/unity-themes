...
namespace Zios.Unity.Editor.Themes{
	...
	public partial class Theme{
		...
		private static bool needsIconRebuild;
		...
		public static void Update(){
			...
			if(Theme.needsReset){Theme.InstantReset();}
				...
				Theme.ClearStyles();
				Theme.ApplyIconset(Theme.needsIconRebuild);
				EditorPref.Call("EditorTheme-Rebuild",Theme.debug);
				Theme.needsRebuild = false;
				Theme.needsIconRebuild = false;
				Theme.Refresh();
				if(Theme.debug){Log.Show("[Themes-Debug] Rebuild -- " + time.Passed());}
			}
				...
				if(Theme.debug){Log.Show("[Themes-Debug] Refresh -- " + time.Passed());}
			}
			else if(!Theme.setup){
				var themes = File.Find("*.unitytheme",Theme.debug);
				if(themes.IsNull()){
		...
		public static void Load(bool lazy=false){
			File.monitor = false;
			var themeName = EditorPref.Build("EditorTheme","Default");
			RelativeColor.autoBalance = EditorPref.Build("EditorTheme-AutobalanceColors",1).As<AutoBalance>();
			Theme.showColorsAdvanced = EditorPref.Build("EditorTheme-ShowAdvancedColors",false);
			Theme.showFontsAdvanced = EditorPref.Build("EditorTheme-ShowAdvancedFonts",false);
			Theme.hoverResponse = EditorPref.Build("EditorTheme-HoverResponse",1).As<HoverResponse>();
			Theme.separatePlaymodeSettings = EditorPref.Build("EditorTheme-SeparatePlaymodeSettings",false);
			Theme.suffix = ProxyEditor.IsChanging() && Theme.separatePlaymodeSettings ? "-Playmode" : "";
			Theme.suffix = "-"+themeName+Theme.suffix;
			var fontset = lazy ? EditorPref.Get<string>("EditorFontset"+Theme.suffix,"Classic") + ".unityFontset" : null;
			var palette = lazy ? EditorPref.Get<string>("EditorPalette"+Theme.suffix,"Classic") + ".unityPalette" : null;
			var iconset = lazy ? File.Find("Iconsets/"+EditorPref.Get<string>("EditorIconset"+Theme.suffix,"Default")) : null;
			var skinset = lazy ? File.Find("Skinsets/"+EditorPref.Get<string>("EditorSkinset"+Theme.suffix,"Default")) : null;
			...
			Theme.themeIndex = Theme.all.FindIndex(x=>x.name==EditorPref.Build("EditorTheme"+Theme.suffix,"Default")).Max(0);
			Theme.active = new Theme().Use(Theme.all[Theme.themeIndex]);
			if(!lazy){
				Theme.fontsetIndex = ThemeFontset.all.FindIndex(x=>x.name==EditorPref.Build<string>("EditorFontset"+Theme.suffix,Theme.active.fontset.name)).Max(0);
				Theme.paletteIndex = ThemePalette.all.FindIndex(x=>x.name==EditorPref.Build<string>("EditorPalette"+Theme.suffix,Theme.active.palette.name)).Max(0);
				Theme.skinsetIndex = ThemeSkinset.all.FindIndex(x=>x.name==EditorPref.Build<string>("EditorSkinset"+Theme.suffix,Theme.active.skinset.name)).Max(0);
				Theme.iconsetIndex = ThemeIconset.all.FindIndex(x=>x.name==EditorPref.Build<string>("EditorIconset"+Theme.suffix,Theme.active.iconset.name)).Max(0);
			}
			EditorPref.Set<string>("EditorSkinset"+Theme.suffix,Theme.active.skinset.name);
			Theme.lazyLoaded = lazy;
		}
		public static void Reset(){Theme.setup = false;}
		...
		public static void CheckUpdate(PlayModeStateChange state){
			if(Theme.separatePlaymodeSettings && !ProxyEditor.IsChanging()){
		...
		public static void UpdateColors(){
			...
			foreach(var color in Theme.active.palette.colors["*"]){
				...
				EditorPref.Set<bool>("EditorTheme-Dark-"+color.Key,color.Value.value.GetIntensity() < 0.4f);
			}
		...
		public static void ApplySettings(){
			...
			foreach(var variant in theme.skinset.variants){
				variant.active = EditorPref.Get<bool>("EditorVariant"+Theme.suffix+"-"+variant.name,false);
			}
			Theme.Apply();
		}
		public static void Apply(string themeName="",bool forceWrite=false){
			...
			var time = Time.Get();
			Theme.ApplySkinset();
			Theme.singleUpdate = Theme.singleUpdate || forceWrite;
			var colors = Theme.active.palette.colors["*"].Where(x=>!x.Value.skipTexture).Select(x=>x.Value).ToList();
			MethodStepSimple<RelativeColor> colorMethod = (color)=>{
				color.UpdateTexture();
				return true;
			};
			Worker.Create(colors,colorMethod.AsIndexed());
			if(Theme.active.palette.swap.Count >= 1){
				...
				MethodStepSimple<FileData> dynamicMethod = (file)=>{
					var texture = default(Texture2D);
					Worker.MainThread(()=>texture=file.GetAsset<Texture2D>());
					Theme.active.palette.ApplyTexture(file.path,texture,Theme.singleUpdate);
					return true;
				};
				Worker.Create(files,dynamicMethod.AsIndexed());
			}
		...
		public static void PrepareFonts(){
			...
			Theme.builtinFonts = Locate.GetAssets<Font>().Where(x=>File.GetPath(x).Contains("Library/unity")).ToArray();
		...
		public static void DrawFonts(){
			...
			if(theme.customizableFontset && hasFontsets){
				...
				foreach(var item in theme.fontset.fonts){
					...
					if(showRenderMode){
						var fontPath = File.GetPath(themeFont.font);
		...
		public static void ApplySkinset(){
			...
			Call.Delay(ProxyEditor.RepaintAll,0.25f);
		}
		//=================================
		// Iconset
		//=================================
		...
		public static void LoadFontset(bool reset=false){
			...
			var value = EditorPref.Get<string>("EditorFontset-Settings"+Theme.suffix,"");
		...
		public static void LoadColors(bool reset=false){
			...
			foreach(var group in theme.palette.colors.Where(x=>x.Key!="*")){
				foreach(var color in group.Value){
					var value = EditorPref.Get<string>("EditorTheme"+Theme.suffix+"-Color-"+group.Key+"-"+color.Key,color.Value.Serialize());
