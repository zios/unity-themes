...
namespace Zios.Unity.Editor.Windows{
	...
	public class ZiosInspector : EditorWindow{
		...
		public UnityEditor.Editor[] editors = new UnityEditor.Editor[0];
		public bool dirty;
		...
		public void OnGUI(){
			//Theme.Apply("Grayson");
			GUI.skin = File.GetAsset<GUISkin>("EditorStyles-Grayson.guiskin");
			var headerStyle = Style.Get("m_InspectorTitlebar").Padding(32,0,4,0);
			var containerStyle = Style.Get("m_InspectorDefaultMargins");
			var cursorArea = new Rect(Event.current.mousePosition.x,Event.current.mousePosition.y,0,0);
		...
		public void OnSelectionChange(){
			...
			this.editors = this.components.Select(x=>UnityEditor.Editor.CreateEditor(x)).ToArray();
			this.dirty = true;
			this.Repaint();
		}
	}
