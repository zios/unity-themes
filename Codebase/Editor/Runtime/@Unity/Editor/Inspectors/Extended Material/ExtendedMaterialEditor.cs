...
namespace Zios.Unity.Editor.Inspectors.ExtendedMaterial{
	...
	public class ExtendedMaterialEditor : MaterialEditor{
		...
		public void Setup(bool full=false){
			if(full){
				this.UI = File.GetAsset<GUISkin>("ExtendedMaterialEditor.guiskin");
			...
			if(targetReady && (MaterialBuffer.refresh || targetMismatch)){
				...
				this.LoadSettings();
				Log.Show("[ExtendedMaterial] Material loaded -- " + material.name + " [" + material.shader.name + "]");
			}
		}
		...
		public static void CheckPreview(){
			if(MaterialBuffer.buildPreview && MaterialBuffer.material != null && Time.Get() > MaterialBuffer.buildDelay){
		...
		public void FixPreviewShader(bool force=false){
			...
			if(force || (MaterialBuffer.material != material && MaterialBuffer.material != null)){
				...
				if(name.Contains("Hidden/Preview/")){
					...
					if(shader == null){
						Log.Warning("Shader for material is a 'preview' shader, but original path could not be found to revert.  Please fix manually.");
						return;
					}
					if(MaterialBuffer.material != null){MaterialBuffer.material.shader = shader;}
					Log.Show("[ExtendedMaterial] Shader reverted -- " + shader.name);
