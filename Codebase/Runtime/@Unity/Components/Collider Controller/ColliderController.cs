...
using System.Linq;
using UnityEngine;
namespace Zios.Unity.Components.ColliderController{
	...
	public class ColliderController : ManagedBehaviour{
		...
		public override void Awake(){
			...
			if(Proxy.IsPlaying()){
				...
				UnityEngine.Time.fixedDeltaTime = this.fixedTimestep;
			}
		}
		...
		private void CheckCollisionEnd(){
			...
			foreach(var collision in this.collisions){
				...
				if(!this.frameCollisions.ContainsKey(target)){
					...
					if(Time.Get() > collisionData.endTime){
						Action method = ()=>{
		...
		private void CheckCollisionStart(){
			foreach(var collision in this.frameCollisions){
				...
				this.collisions[collision.Key].endTime = Time.Get() + this.collisionPersist;
			}
		}
		...
		private void CheckBlocked(){
			...
			if(this.blocked.forward){this.lastBlockedTime.forward.Set(Time.Get());}
			if(this.blocked.back){this.lastBlockedTime.back.Set(Time.Get());}
			if(this.blocked.up){this.lastBlockedTime.up.Set(Time.Get());}
			if(this.blocked.down){this.lastBlockedTime.down.Set(Time.Get());}
			if(this.blocked.right){this.lastBlockedTime.right.Set(Time.Get());}
			if(this.blocked.left){this.lastBlockedTime.left.Set(Time.Get());}
		}
