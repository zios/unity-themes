...
namespace Zios.Unity.Components.PC2{
	...
	public class PC2Controller : MonoBehaviour{
		...
		public void OnValidate(){
			...
			if(this.previous != this.file){
				...
				this.path = File.Get(this.file).path;
		...
		public void Update(){
			if(!this.ready){return;}
			if(Time.Get() > this.nextFrameTime){
				...
				this.nextFrameTime = Time.Get() + 1.0f/speed;
				this.currentFrame += 1;
			}
		}
		public void OnDrawGizmosSelected(){
			...
			if(dataReady && meshReady){
				...
				if(!Proxy.IsPlaying() && this.showOriginalVertexes){
	...
	public class PC2Data{
		...
		public void Load(string path){
			try{
				this.frames.Clear();
				using(BinaryReader binary = new BinaryReader(System.IO.File.Open(path,FileMode.Open))){
				...
				Log.Show("[PC2Data] : File Successfully parsed -- " + path);
			}
			catch{
				Log.Warning("[PC2Data] : Error parsing file -- " + path);
			}
		}
	}
	[Serializable]
