...
namespace Zios.Unity.SpriteManager{
	...
	public class SpriteController : MonoBehaviour{
		...
		[NonSerialized] public Sprite instance;
		[NonSerialized] public float frame = -1;
		...
		public void Start(){
			...
			if(Proxy.IsPlaying() && this.instance == null && this.spriteXML != null){
		...
		public void Load(){
			...
			if(this.sequences.ContainsKey(fullName)){
				...
				if(this.instance == null){
					Log.Show("[SpriteController] ^2Error loading sprite -- ^7" + this.spriteName + "-^4" + this.spriteAnimation);
					return;
				}
				this.Begin();
			}
			else{
				Log.Show("[SpriteController] No sprite named " + fullName + " was found.");
			}
		}
		...
		public void Reset(){
			if(this.instance != null){
				...
				if(this.spriteRandomStart){
					int random = UnityEngine.Random.Range(0,this.instance.frames.Count-1);
		...
		public void Update(){
			if(Proxy.IsPlaying() && this.instance != null && this.spriteAnimation != ""){
		...
		public void UpdateFrame(){
			...
			this.frame = this.instance.currentFrame;
			this.nextUpdate = Time.Get() + ((1000.0f / this.spriteSpeed) / 1000.0f);
		}
