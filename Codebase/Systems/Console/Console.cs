...
namespace Zios.Console{
	...
	public partial class Console : Singleton{
		...
		public void OnDisable(){Proxy.RemoveLogCallback(Console.HandleLog);}
		...
		public void Start(){
			...
			Console.LoadBinds();
			Call.Delay(()=>Console.LoadConfig(this.configFile));
		}
	}
	public partial class Console{
		...
		public static void CheckHotkeys(){
			...
			if(Button.EventKeyDown(KeyCode.Return)){
				...
				if(control){Console.Get().logFontSize -= (int)Event.current.delta[1];}
				else if(shift){Console.Get().height += Math.Sign(Event.current.delta[1]) * 0.02f;}
		...
		public static void ManageState(){
			float slideStep = Console.Get().speed * Time.GetDelta();
		...
		public static void ResetValue(string[] values){
			...
			if(Console.binds.ContainsKey(name,true)){
				...
				Console.AddLog("^3|" + data.value.name + "^10 has been reset to its default value ^7-- ^8|" + data.defaultValue);
			}
			else{
